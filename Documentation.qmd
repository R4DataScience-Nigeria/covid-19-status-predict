...
# Handle missing values
policy_df <- policy_df %>%
  filter(!is.na(confirmedcases) & !is.na(confirmeddeaths))
```
...
# Handle missing values
tracking_df <- tracking_df %>%
  filter(!is.na(positive) & !is.na(negative) & !is.na(hospitalized) & !is.na(death))
```
...
You can copy this content into a `.qmd` file for your project. If you need any further assistance or have any questions, feel free to ask!
